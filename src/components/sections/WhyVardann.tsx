...
export default function WhyVardann() {
  return (
    <section className="relative overflow-hidden bg-soft-blue py-20 sm:py-28">
      <div
        aria-hidden="true"
      ...
      <div className="relative mx-auto max-w-6xl px-6 lg:px-10">
        ...
        <div className="mt-14 grid gap-6 sm:grid-cols-2">
          ...
          <div className="rounded-2xl border border-vblue/10 bg-white p-8 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-vblue/40 hover:shadow-xl">
            <p className="text-eyebrow text-[0.68rem] text-gold">Mission</p>
        ...
        <div className="mt-10">
          ...
          <div className="mt-4 flex flex-wrap gap-2.5">
            {industriesServed.map((ind) => (
              <span
                key={ind}
                className="rounded-full border border-vblue/15 bg-white px-4 py-1.5 text-sm text-body shadow-sm transition-colors duration-200 hover:border-gold hover:text-navy"
              >
                {ind}
              </span>
            ))}
